<template>
  <div class="per-box">
    <Top :HRight='HRight' :HTitle='HTitle' :toUrl='toUrl' @Click='logOut'></Top>
    <div class="per-form">
      <div class="myLi">
        <div v-show='!editor'>
          <span class="leftTitle">{{farmName}} <span style="font-weight: normal; font-size: .28rem;">({{userType == 1? '养殖场':'个体' }})</span> </span>
          <img @click='editorName' class="editorIMG" :src="i_editor_u" alt="">
        </div>
        <div class="input_box" v-show='editor'>
          <input v-model='newName' class="input" type="text" name="">
        </div>
      </div>
      <div  v-show='!editor && farm' style="text-align: left; height: .5rem;"><span class="leftTitle" v-show='!editor' style="line-height: .3rem;font-size: .3rem;    float: left;   ">饲料名称：{{food?food:'未喂养饲料'}}</span></div>
      <div v-show='editor && farm' class="myLi" >
        <div class="input_box"  style="top: 2.2rem">
          ...
          <input v-model='newFood' class="input" type="text" name="">
        </div>
      </div>
    </div>
    ...
    <div class="per-form sth-form">
      ...
      <div class="mtfooter" v-show='editor'>
        <SmallBtn :btnName='btnName1' @Click='sure'></SmallBtn>
        <SmallBtnNo :btnName='btnName2' @Click='cancel' style='margin-left: .3rem;'></SmallBtnNo>
      </div>
...
export default {
  ...
  methods: {
    ...
    sure() {
      ...
      let vm = this
      if(vm.strLen(vm.newName) > 16){
        vm.$msg('名称不能超过16个字符串')
      }else if (0 < vm.strLen(vm.newName) &&  vm.strLen(vm.newName) <= 16 && vm.newFood != '') {
    ...
    goPhone() {
      this.$router.push('/Main/BindPhone')
    },
    goPulPs() {
